...
def dist(L1, L2):
	sum = 0	
	for i in range(4):
		sum += (L1[i]-L2[i])*(L1[1]-L2[i])
	return np.sqrt(sum)

class Persons:
	def __init__(self):
		posi = np.random.randint(0,600,2)
		self.person = canvas.create_oval(posi[0], posi[1], posi[0]+10,10+posi[1], fill = "green")
...
L5 = Label(frame,text = "Death:", width = 0).pack(side = LEFT)
E5 = Entry(frame,textvariable = Inf_var, width = 3).pack(side = LEFT)
